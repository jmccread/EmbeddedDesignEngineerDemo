 ...
 *                  
 *************************************************************************************/
uint8_t start_device_uart_A1(uint32_t baud_rate) {
  uint8_t error;
  ...
  USCI_A_UART_enableInterrupt (&UART_A1, USCI_A1_UART_RECEIVE_INTERRUPT);
  USCI_A_UART_enableInterrupt (&UART_A1, USCI_A1_UART_TRANSMIT_INTERRUPT);

  }

uint8_t start_device_uart_A0(uint32_t baud_rate ) {
  uint8_t error;
  // Enable UART functionality (TO DO: Elegant clock)
  __enable_interrupt();
   error = USCI_A_UART_init (&UART_A0,
                    USCI_A_BASE,
                    USCI_A_UART_CLOCKSOURCE_SMCLK,
                    SMCLK_SPD,
                    baud_rate,
  ...
  //enable UART Pins (TO DO: Change?)
  USCI_A_UART_enable (&UART_A0);
  P3SEL |= BIT4 + BIT5;
  P3DIR |= BIT6;

  //Enable UART interrupts
  USCI_A_UART_enableInterrupt (&UART_A0, USCI_A0_UART_RECEIVE_INTERRUPT);
  USCI_A_UART_enableInterrupt (&UART_A0, USCI_A0_UART_TRANSMIT_INTERRUPT);
